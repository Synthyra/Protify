...
The `--plan_only` form emits the synthetic plan and static estimates without resolving a device, allocating synthetic tensors, or training.
...
- Treat shared minibatch order as a prototype choice: this maximizes shared I/O and arithmetic intensity, but it is not bit-equivalent to the current sequential path where each seed can get an independent shuffle.
- Validate downstream metrics are statistically comparable across seeds before enabling this by default.
