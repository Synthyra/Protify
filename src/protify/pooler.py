...
class Pooler:
    ...
    def eos_pooling(
            ...
            **kwargs,
        ) -> torch.Tensor: # (b, L, d) -> (b, d)
        """Pool an explicit boundary token or the final active token.

        ``attention_mask.sum() - 1`` only works for right-padded contiguous masks.
        Taking the maximum active position also supports left padding and masks
        with internal gaps. When the tokenizer exposes an explicit pooling token,
        require exactly one active occurrence so a missing or duplicated boundary
        cannot silently select the wrong representation.
        """
        if emb.ndim != 3:
        ...
        if attention_mask is None:
            ...
            active_mask = attention_mask.to(device=emb.device, dtype=torch.bool)
        ...
        if eos_token_id is not None:
            ...
            explicit_eos = input_ids.to(device=emb.device).eq(int(eos_token_id)) & active_mask
            eos_counts = explicit_eos.sum(dim=1)
            ...
            selected_idx = explicit_eos.to(dtype=torch.long).argmax(dim=1)
        else:
            positions = torch.arange(sequence_length, device=emb.device).expand(batch_size, -1)
            selected_idx = positions.masked_fill(~active_mask, -1).amax(dim=1)

        return emb[torch.arange(batch_size, device=emb.device), selected_idx]
    ...
    def __call__(
        ...
        for pooling_type in self.pooling_types:
            final_emb.append(self.pooling_options[pooling_type](
                emb=emb,
